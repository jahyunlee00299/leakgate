...
gitleaks·maskingtape·ko-pii를 추가 엔진으로 붙일 수 있습니다.

- **문서 내부까지**: docx·xlsx·pptx·hwpx·odt의 본문뿐 아니라 변경 내용 추적으로 지운 글자,
  메모, 머리글·바닥글, 문서 속성의 작성자까지 봅니다. PDF는 `leakgate[pdf]`, 이미지는 `--ocr`.
  읽지 못한 파일(구형 .hwp/.doc, 텍스트 없는 스캔 PDF 등)은 조용히 넘기지 않고 exit 2로 알립니다.
- **아는 사람 명단**: `names`에 등록한 이름은 문장 어디에 있든 잡습니다(`김민준이 보냈다`).
- **git 히스토리**: `scan --history`로 과거 커밋에서 지운 키까지 찾고,
  `hook install`로 push 전에 막습니다.
- **정직한 수치**: 규칙을 본 적 없는 세트(heldout3)에서 시크릿 49/60, 한국 PII 46/70,
  인프라 23/30, 오탐 0/110. 남은 누락은 대부분 명단에 없는 사람 이름입니다.

유출된 키는 가리는 것만으로 안전해지지 않으니 반드시 폐기·재발급하세요.
